    ...
    return asyncAuto({
      ...
      description: ['getAliases', ({getAliases}, cbk) => {
        ...
        const path = route.hops.map((hop, i, hops) => {
          ...
          const isBlinded = isBlindedHop(hop);
          const isFinal = i === hops.length - 1 && hops.length !== 1;
          const pair = [(hops[i - 1] || {}).public_key, hop.public_key].sort();

          // Blinded hops have no channel, timeout or amount of their own
          const channel = !isBlinded ? hop.channel : blindedChannel;
          const edge = isBlinded ? '' : pairEdgeIndex(pair, hop.public_key);
          const feeMtokens = isFinal ? hops[i-1].fee_mtokens : hop.fee_mtokens;
          const forwarder = `${aliasColor(alias)} ${hop.public_key}`.trim();
          const hasForward = !!BigInt(hop.forward_mtokens || Number());
          const prior = hops[i - 1];

          const delta = !i || isBlinded ? null : prior.timeout - hop.timeout;
          const expiry = delta > 240 ? redBright(` Expiry +${delta}`) : '';

          // The rate is unknown when the hop forward amount is unknown
          const rate = !hasForward ? null : formatFeeRate({
            rate: effectiveFeeRate(feeMtokens, hop.forward_mtokens),
          }).display;

          const forward = rate === null ? forwarder :
            `${forwarder}. Fee rate: ${rate}${expiry}`;

          if (!i) {
            return [`${gray(channel)} ${description || String()}`, forward];
          } else if (i === hops.length - [i].length) {
            return [`${gray(channel + edge)}`];
          } else {
            return [`${gray(channel + edge)}`, forward];
          }
        });
