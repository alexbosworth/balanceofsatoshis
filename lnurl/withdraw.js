...
module.exports = (args, cbk) => {
  return new Promise((resolve, reject) => {
    return asyncAuto({
      ...
      validate: cbk => {
        ...
        try {
          ...
          return cbk([400, 'FailedToDecodeLnurlToWithdraw']);
        }
      ...
      getTerms: ['validate', ({}, cbk) => {
        ...
        return args.request({url, json: true}, (err, r, json) => {
          ...
          if (json.tag !== tag) {
            return cbk([503, 'ExpectedTagToBeWithdrawRequestInLnurlResponseJson']);
          }
      ...
      askAmount: ['getTerms', ({getTerms}, cbk) => {
        ...
        return args.ask({
          ...
          validate: input => {
            ...
            if (Number(input) < min) {
              return `Service min withdrawable is ${min}, try a higher amount?`;
            }
      ...
      withdraw: ['createInvoice', 'getTerms', ({createInvoice, getTerms}, cbk) => {
        args.logger.info({
        ...
        const qs = {
          k1,
          pr: createInvoice.request,
        };

        return args.request({url, qs, json: true}, (err, r, json) => { 
          if (!!err) {
            return cbk([503, 'FailureToWithdrawFromLnurl', {err}]);
          }

          if (!json) {
            return cbk([503, 'ExpectedJsonObjectReturnedInLnurWithdrawResponse']);
          }
          ...
          if (json.status === 'OK') {
            args.logger.info({is_withdrawal_success: true});
          }
    ...
    returnResult({reject, resolve, of: 'withdraw'}, cbk));
  });
};
